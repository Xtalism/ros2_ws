...
class MonoDriver(Node):
    def __init__(self, node_name="mono_py_node"):
        ...
        self.home_dir = (
            str(Path.home()) + "/ros2_ws/src/ros2_orb_slam3"
        ...
        self.image_sequence_dir = (
            self.home_dir + "/" + self.parent_dir + "/" + self.image_seq
        )  # Full path to the image sequence folder
        ...
        self.send_config = (
            True  # Set False once handshake is completed with the cpp node
        )

        # * Setup ROS2 publishers and subscribers
        self.publish_exp_config_ = self.create_publisher(
            String, self.pub_exp_config_name, 1
        )  # Publish configs to the ORB-SLAM3 C++ node
