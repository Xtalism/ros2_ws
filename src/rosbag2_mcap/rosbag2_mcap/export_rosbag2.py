...
class export_rosbag2:
    ...
    def get_field_value(self, msg, field_name: str) -> float | str:
        try:
    ...
    def stats_rosbag2(self, args) -> tuple:
        timestamps = []
        values = []

        for topic, msg, timestamp in self.read_messages(args.input):
            if topic == args.topic:
                try:
                    time_seconds = timestamp / 1e9
                    field_value = self.get_field_value(msg, args.field)
    ...
    def graph_rosbag2(self, args, stats):
    ...
    def csv_rosbag2(self, args, stats):
        if stats is None:
            return
...
def main():
    exporter = export_rosbag2()
    
    args = exporter.parse_args()
    stats = exporter.stats_rosbag2(args)
    
    if args.graph:
        exporter.graph_rosbag2(args, stats)
    elif args.csv:
        exporter.csv_rosbag2(args, stats)
    elif not args.field or not args.topic:
        raise ValueError("Error: Please specify both --topic and --field options")
    else:
